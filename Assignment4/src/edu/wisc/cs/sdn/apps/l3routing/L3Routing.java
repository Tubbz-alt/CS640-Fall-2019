		...
		ILinkDiscoveryListener, IDeviceListener
{
	...
	public void installRules() {
		for (Map.Entry<Long, Map<Long, Graph.LinkDistancePair>> entry : this.graph.getTable().entrySet()) {
			...
			IOFSwitch currentSwitch = this.floodlightProv.getSwitch(currentSwitchId);
			...
			for (Host host : getHosts()) {
				...
				instruction.setActions(Arrays.asList(actionOutput));

				SwitchCommands.removeRules(currentSwitch, table, matchCriteria);
				SwitchCommands.installRule(currentSwitch, table, SwitchCommands.DEFAULT_PRIORITY, matchCriteria, Arrays.asList(instruction));
			}
		}
	}
